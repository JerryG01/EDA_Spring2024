...
EPAair <- 
  read.csv(here("Data/Processed_KEY/EPAair_O3_PM25_NC1819_Processed.csv"),
...
EPAair.sf <- EPAair.sites %>% 
  st_as_sf(coords = c('mean_Lng','mean_Lat'), #specify coordinate, x first
           crs=4326) #specify coordinate system, which is earth
...
sfc_GraingerHall <- st_sfc(sfg_GraingerHall,crs=4326) #sfc: simple feature column
class(sfc_GraingerHall)
mapview(sfc_GraingerHall)

#Upgrade to a spatial dataframe, that also shows attributes
...
#spend out with the unit of measure of 1 unit (1 degree of latitude and longtitude)
...
EPAair.sf.utm <- st_transform(EPAair.sf,32617) # Re-project by meters
