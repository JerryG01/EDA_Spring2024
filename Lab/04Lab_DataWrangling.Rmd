...
```{r}
year1999 <- filter(NTL.phys.data, year4 == 1999)
tuesday90_99 <- 
...
```{r}
help(as.Date)
NTL.phys.data$sampledate <- as.Date(NTL.phys.data$sampledate, "%Y-%m-%d")
month <- month(NTL.phys.data$sampledate)
tlake90_99_july <-
  NTL.phys.data %>%
...
```
```{r}
remove(surface_tlake90_99_july)
surface_tlake90_99_july <-
  tlake90_99_july %>%
  filter(depth == 0) %>%
