...
Rainfall_BR_processed <-
  ...
  separate(Statistics,c("Null","Month","Null2")," ") %>% # “” is the seperator
  mutate( Date = my(paste0(Month,"-",Year))) %>% # my stand for "month year" and will recreate a variable that contains date; Paste0 means there is no sperator like " " 
...
# Inner_join can merge the two data sets processed together, in this case, they will be join via "Date"; it works better when there are potential missing data than c-bind
...
rainfall_plot <- ggplot(BR_complete, 
  aes(x = Date, y = Rainfall)) + 
  geom_smooth() + 
  geom_line()
print(rainfall_plot)

temp_plot <- ggplot(BR_complete, )
...
# To use below formula to extract year/month, we have to make sure the first row is the first observation
first_month <- month(first(BR_complete$Date))
first_year <- year(first(BR_complete$Date))
...
# Exercise 8: Use the stl() function to decompose both series. Seasonal Deconposition Trend
# Visualize the decomposed series using plot(). What components can you identify?

decomp_rain <- stl(BR_rain_ts, s.window = "periodic")
decomp_temp <- stl(BR_temp_ts, s.window = "periodic")
...
df_rain_components <- as.data.frame(decomp_rain$time.series) %>%
  mutate(Observed = BR_complete$Rainfall, 
         Date  = BR_complete$Date)
df_temp_components <- as.data.frame(decomp_temp$time.series) %>%
  mutate(Observed = BR_complete$Temperature_C, 
         Date  = BR_complete$Date)

# Rename the columns for clarity
colnames(df_rain_components) <- c("Seasonal", "Trend", "Remainder")
colnames(df_temp_components) <- c("Seasonal", "Trend", "Remainder")

# Exercise 10: Visualize how the trend maps onto the data for both series
rain_comp_plot <- ggplot(df_rain_components, aes(x = Date)) +
  geom_line(aes(y = Observed), color = "black") +
  geom_line(aes(y = trend), color = "blue") +
  ylab("Rainfall") + 
  mytheme
print(rain_comp_plot)

temp_comp_plot <- ggplot(df_temp_components, aes(x = Date)) +
  geom_line(aes(y = Observed), color = "black") +
  geom_line(aes(y = trend), color = "blue") +
  ylab("Temperature") + 
  mytheme
