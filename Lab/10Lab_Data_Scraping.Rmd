...
gage_data <- read.table(theURL,
                        skip = 29,   #skip the first 29 rows
                        header=TRUE, #there are headers in the origional dataset
                        sep='\t',    #sperator is 'tab'
                        stringsAsFactors = T) 
...
nc_pop <- 
  get_acs(geography = "county",
          ...
          state = "NC",
          geometry = TRUE) # Return just the table
...
the.df <- data.frame(
  ...
  capacity = gsub(",", "", the_capacity) %>% as.numeric(), 
  #Substitude "," with '', basically nothing, and then turn them into numeric
  net.generation = gsub(",", "",net_generation) %>% as.numeric(),
  total.retail = gsub(",", "",total_retail) %>% as.numeric()
  )
```
